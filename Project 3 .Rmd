...
mouseAAseq <- AAString(mProseq) 
humanAAseq <- AAString(hProseq)
...
dotPlot(mPPARARNAseq, hPPARARNAseq) #Nucleotide similarity visualization
dotPlot(mPPARAProseq, hPPARAProseq) #Amino acid similarity visualization
```
We can see that there is a lot of noise in the DNA sequence, likely because the window and stringency are too low and there likely are multiple repeats present across the sequences, thus causing a very "noisy" dotplot. On the otherhand for the amino acid sequence, there is a relatively clear line that can be followed, indicating that there is a big similarity between the mouse and human PPARA amino acid sequence. 

_____________________________________________________________________________________

Analysis: While the PPARA DNA sequence percent similarity of mice and humans is somewhat low (78%), the PPARA amino acid sequence percent similarity is high (94%), and these results are somewhat reflective in the dotplots, where the DNA dotplot is unclear with no apparent trends while the amino acid dotplot has a very clear linear trend showing the mouse and human amino acid sequences are very similar. Although the DNA similarity is low, I would still conclude that the human and mouse PPARA receptors are very similar due to the amino acid sequence similarity because different combinations of 3 RNA bases (which is related to DNA) can code for the same amino acid, and thus yield the same overall amino acid despite having different DNA and RNA sequences. In addition, protein function and chemistry is determined by the amino acid sequences, and thus since the sequences are similar, it is highly likely that the human and mouse PPARA have similar chemistry. Thus, 10(Z)-hexadecenoic acid should have a similar agonistic effect on human PPARA receptors like it does on mouse PPARA receptors, and produce an anti-inflammatory response in human macrophages that is similar to mouse macrophages.
